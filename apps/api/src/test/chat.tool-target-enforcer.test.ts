...
describe('POST /chat — tool target enforcement', () => {
  ...
  // ── Case 5b: verbatim clarification reply → resolves to single same-company match ──
  it('resolves to a single match when the user replies verbatim with "title at company"', async () => {
    buildSupabaseMock({
      userId: TARGET_USER,
      tier: 'pro',
      entitlementTier: 'pro',
    ...
    mockCallLLMWithToolResult.mockResolvedValueOnce({
      type: 'text',
      content: 'Here is the gap analysis for the Senior Engineer role at Acme.',
      provider: 'anthropic',
    })
    ...
    const res = await app.request('/chat', {
      ...
      body: JSON.stringify({
        ...VALID_BODY,
        userId: TARGET_USER,
        // Verbatim label from the clarification prompt
        message: 'Senior Engineer at Acme',
      }),
    })
    ...
    expect(doneEvent?.['message']).toBe(
      'Here is the gap analysis for the Senior Engineer role at Acme.',
    )
    expect(mockRunWorkerGapAnalysis).toHaveBeenCalledTimes(1)
    expect(mockCallLLMWithToolResult.mock.calls[0]?.[4]).toEqual({ job_id: 'job-acme-001' })
  })
